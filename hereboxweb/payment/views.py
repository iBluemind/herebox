...
def reservation_payment():
    ...
    if request.method == 'POST':
        ...
        pay_type = '현장' if new_reservation.pay_type == PayType.DIRECT else '온라인'
        send_mms.apply_async(args=['01064849686', u'[히어박스] 주문번호: %s / 이름: %s / 예약시간: %s %s / 유형: 신규 / 결제방법: %s'
        ...
        if new_revisit_schedule:
            ...
            send_mms.apply_async(args=['01064849686', u'[히어박스] 주문번호: %s / 이름: %s / 예약시간: %s %s / 유형: 신규 / 결제방법: %s'
                                           ...
                                           pay_type, new_revisit_schedule.address, current_user.phone,
                                           new_revisit_schedule.standard_box_count,
                                           new_revisit_schedule.nonstandard_goods_count, new_revisit_schedule.period,
                                           new_revisit_schedule.user_memo)])
        return response_template(u'정상 처리되었습니다', 200)
